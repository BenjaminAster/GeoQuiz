		...
		});
		{
			...
			canvas.addEventListener("touchmove", (event) => {
				if (event.touches.length === 2) {
					...
					const y = midpointY - canvas.parentElement.offsetTop;
		...
		const draw = () => {
			...
			if (settings.capital) {
				...
				mouseHoverLoop: for (const country of [...data].reverse()) {
					if (Math.hypot((country.capitalCoordinates[1] - mousePointX), (country.capitalCoordinates[0] - mousePointY)) < radiusPoints) {
